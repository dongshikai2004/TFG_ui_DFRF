...
class Face_3DMM(nn.Module):
    def __init__(self, modelpath, id_dim, exp_dim, tex_dim, point_num):
        ...
        for i in range(3):
            mu[:, i] -= np.mean(mu[:, i])
        mu = mu.reshape(-1)
        self.base_id = torch.as_tensor(base_id).cpu()/100000.0
        self.base_exp = torch.as_tensor(base_exp).cpu()/100000.0
        self.mu = torch.as_tensor(mu).cpu()/100000.0
        base_tex = DMM_info['b_tex'][:tex_dim, :]
        mu_tex = DMM_info['mu_tex']
        self.base_tex = torch.as_tensor(base_tex).cpu()
        self.mu_tex = torch.as_tensor(mu_tex).cpu()
        ...
        self.sig_tex = torch.as_tensor(sig_tex).cpu()
        self.sig_exp = torch.as_tensor(sig_exp).cpu()

        keys_info = np.load(os.path.join(
            modelpath, 'keys_info.npy'), allow_pickle=True).item()
        self.keyinds = torch.as_tensor(keys_info['keyinds']).cpu()
        self.left_contours = torch.as_tensor(keys_info['left_contour']).cpu()
        self.right_contours = torch.as_tensor(
            keys_info['right_contour']).cpu()
        self.rigid_ids = torch.as_tensor(keys_info['rigid_ids']).cpu()
