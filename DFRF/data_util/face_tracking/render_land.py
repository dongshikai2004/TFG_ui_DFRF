...
class Render_Land(nn.Module):
    def __init__(self):
        ...
        self.lands_info = torch.as_tensor(lands_info).cpu()
        tris = np.loadtxt('../data/3DMM/tris.txt', dtype=np.int64)
        self.tris = torch.as_tensor(tris).cpu() - 1
        vert_tris = np.loadtxt('../data/3DMM/vert_tris.txt', dtype=np.int64)
        self.vert_tris = torch.as_tensor(vert_tris).cpu()
        self.normal_baser = Normal_Base().cpu()
        self.renderer = Render_RGB().cpu()
